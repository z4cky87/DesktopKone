...
{
    public class ExportDailyOutbound : ReportProperty
    {
        public List<ReportDailyOutbound> myReport = new List<ReportDailyOutbound>();
        private string[] DataKolom = new string[] { "DATE", "OUTGOING CALL", "OUTBOUND CALL", "AVG OUTBOUND CALL" };
        ...
        public void GenerateReport(string fileName)
        {
            try
            {
                using (ExcelPackage excelPackage = new ExcelPackage())
                {
                    ...
                    SetWorkbookProperties(excelPackage, "JSM", "Report DAILY OUTBOUND");
                    //Create a sheet
                    ExcelWorksheet excelWorkSheet = CreateSheet(excelPackage, "Report HOURLY OUTBOUND");


                    //Merging cells and create a center heading for out table
                    excelWorkSheet.Cells[1, 1].Value = "REPORT DAILY OUTBOUND";
        ...
        private void CreateData(ExcelWorksheet excelWorkSheet)
        {
            ...
            for (int i = 0; i < myReport.Count; i++)
            {
                ...
                { excelWorkSheet.Cells[i + 3, 3].Value = new TimeSpan(00, 00, 00); }
                else
                {
                    excelWorkSheet.Cells[i + 3, 3].Value = new TimeSpan(Convert.ToInt16(myReport[i].outboundcalltime.Substring(0, 2)),
                     Convert.ToInt16(myReport[i].outboundcalltime.Substring(3, 2)), Convert.ToInt16(myReport[i].outboundcalltime.Substring(6, 2)));
                }
                excelWorkSheet.Cells[i + 3, 3].Style.Numberformat.Format = "[h]:mm:ss";

                if (string.IsNullOrEmpty(myReport[i].avg_outboundcalltime))
                { excelWorkSheet.Cells[i + 3, 4].Value = new TimeSpan(00, 00, 00); }
                else
                {
                    excelWorkSheet.Cells[i + 3, 4].Value = new TimeSpan(Convert.ToInt16(myReport[i].avg_outboundcalltime.Substring(0, 2)),
            ...
            var cell = excelWorkSheet.Cells[3, 1, myReport.Count + 2, 4];
