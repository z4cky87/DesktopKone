...
namespace Contact_Center_Kone.Report
{
    public class ExportDailyInbound : ReportProperty
    {
        ...
        private string[] DataKolom = new string[] { 
            ...
            "TOTAL ACW", "TOTAL AVG ACW", "TOTAL BLANKCALL", "TOTAL WRONGNO", "TOTAL INQUIRY", "TOTAL COMPLAIN", "TOTAL REQUEST", "TOTAL TEST CALL", "TOTAL OTHERS" };
        ...
        private void CreateData(ExcelWorksheet excelWorkSheet)
        {
            ...
            for (int i = 0; i < myReport.Count; i++)
            {
                ...
                excelWorkSheet.Cells[i + 3, 14].Value = myReport[i].total_request;
                excelWorkSheet.Cells[i + 3, 15].Value = myReport[i].total_testcall;
                excelWorkSheet.Cells[i + 3, 16].Value = myReport[i].total_others;
            }
            // ws.Column(2).Style.Numberformat.Format = "yyyy-MM-dd";
            var cell = excelWorkSheet.Cells[3, 1, myReport.Count + 2, 16];
